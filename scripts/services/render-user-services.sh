...
resolve_bin_path() {
  local raw_path="$1"
  if [[ "$raw_path" = /* ]]; then
    [[ -x "$raw_path" ]] || fail "file-snitch binary is not executable: $raw_path"
    printf '%s\n' "$raw_path"
    return
  fi
  ...
  [[ -x "$resolved" ]] || fail "file-snitch binary is not executable: $resolved"
  printf '%s\n' "$resolved"
}
...
render_template() {
  ...
  python3 - "$template_path" "$destination_path" "$log_dir" "$resolved_bin_path" <<'PY'
import pathlib
import sys
