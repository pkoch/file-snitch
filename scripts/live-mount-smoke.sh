...
verify_audit_log() {
  local -a audit_patterns=(
    ...
    'existing-note.txt.tmp -> /existing-note.txt'
    '.hidden-temp-note.txt.tmp -> /hidden-temp-note.txt'
    '/backup-note.txt -> /backup-note.txt~'
    '.backup-note.txt.swp -> /backup-note.txt'
    '"action":"truncate"'
    '"action":"unlink"'
    '"action":"chown"'
