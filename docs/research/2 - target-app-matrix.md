...
| GitHub CLI | `$XDG_CONFIG_HOME/gh/` or `~/.config/gh/` | directory | yes | no write observed | none observed in traced run | none observed in traced run | yes | Linux mounted-run captured | `gh auth status --json hosts` on Ubuntu 25.10 arm64 under Lima, with `GH_CONFIG_DIR` pointed at a mounted config dir, opened and read both `config.yml` and `hosts.yml` as `/usr/bin/gh`. No writes, renames, chmod/chown, or lock-heavy behavior were observed beyond ordinary flush/lock callbacks. |
