...
main() {
  ...
  grep -F 'hint: start `file-snitch agent --foreground` or install the per-user agent service' <<<"$doctor_output" >/dev/null || fail "expected doctor to explain how to fix a missing agent socket"
