...
- Added an in-memory guarded-store backend and backend metadata/listing hooks
...
- Fixed Linux release artifact builds with Zig 0.16 and glibc fortify enabled.
...
- Changed the projection state directory to honor `XDG_STATE_HOME`, falling
...
- Fixed `doctor` service checks so absent or stale service files do not fail
  because unrelated host-loaded user services are stale.
...
- Removed permissive legacy input aliases for prompt outcomes, linux-ui
  selections, policy expiry nulls, and the `file-snitch -V` version spelling.
